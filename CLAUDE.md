...
Rama **`claude/port-world-cup-design-FvZpD`** HEAD `e048815` — **F3 fundamentos integración cerrados (15 may PM)**. F2.9 funcional cerrada previa (14 HFs + HF-cierre doc). Sesión 15-may PM 4 commits: `f509a82` audit nomenclatura canónica v3GruposMount/v3ElimMount + `d6bae7c` I1 routing `ui-nav.showPage` dispatch `mundial:page-changed` + `e1de51f` **I1.5 retroactivo** carga assets v3 en SPA (F2.x estaba sandbox-only; sin esto I1 era no-op) + `e048815` I1.6 cleanup `#page-grupos` legacy (-61 LOC) + chips ADMIN/logout en shell v3. Smoke OK: v3 renderiza donde debe. **5 bugs UX** (próxima sesión): hueco GROUP STAGE↔board, "Clasificación" post-simulación rompe estética grupos, elim sin banderas/formato, chip logout no captura `do-logout` legacy (I1.6.2 ~5 LOC), posible null-deref `scoring.js:1239/1315` (I1.6.1 ~5 LOC). **Lección F2.x sandbox-only ≠ SPA** persistida en audit doc + esta entrada. **NO mergeado a main**. Rama paralela `sync/ef-get-squad-v6` (squads 7/48) abierta. Próximo: bugs UX I1.6.x + I3 event bus + I4 close-porra + I1.7 cleanup `#page-elim`.

## Top-3 pendientes inmediatos

1. **Quick wins F3-I1.6.x + bugs UX visuales** (~15 LOC + CSS tweaks): I1.6.2 chip logout (`onclick="doLogout()"` o expose window) · I1.6.1 null-guards `scoring.js:1239` (`total-points`) y `:1315` (`groups-container`) si TypeError confirmado · hueco stage↔board grupos · "Clasificación" rota post-simulación · banderas/formato amigable en elim simulado.
2. **F3-I3 event bus + F3-I4 close-porra** (I4 bloqueador 11 jun): (I3) `mundial:predictions-changed` disparado en `savePredictions()` legacy + listeners re-render v3 ~60 LOC; (I4) read `_porraIsClosed` en cards v3 → read-only post-kickoff ~50 LOC. Detalle: `docs/AUDIT_LEGACY_VS_V3.md` tabla transversales.
3. **F3-I1.7 cleanup `#page-elim`** + **HF-08 propagación grupos→KO**: análogo I1.6 sobre elim (fc-elim mounts F7.X.4 + view-cinematic/bracket/stadium + finalizar-section + modal — verificar `#total-ko-pts` / `updateKOPts` antes de eliminar). HF-08 5 bloques A-E en `docs/AUDIT_LEGACY_VS_V3.md` Backlog F3, 3-5h estimadas.
