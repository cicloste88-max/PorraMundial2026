...
## Hallazgos validados pre-Code (Claude.ai 20-may, end-to-end con HTML real)

Antes de pasar el brief, ejecuté las piezas 0+A+B en local contra:
1. HTML completo de page 1 de la masiva FIWC (30 filas reales)
2. HTML completo de `/spanien/kader/verein/3375/plus/1` (27 jugadores españoles)

**Cobertura observada tras los fixes**:

| Campo | A (page 1 real) | B (España kader real) |
|---|---|---|
| tm_player_id, nombre, position_tm, valor, foto, iso3, verein_id, slug | 30/30 | 27/27 |
| edad, club, club_id, club_logo_url | 25/30 (top 5 sin) | 27/27 |
| dorsal, dob | n/a | 27/27 |

**Caso especial — TOP 5 jugadores de la masiva**: las filas de los 5 jugadores con MÁS valor de mercado tienen estructura reducida (solo foto+nombre+selección+valor, SIN edad/club). Esto significa:

- A da info parcial incluso para top mundial
- **B (kader) es necesario aunque coverage_A_matched sea alto**
- Por eso `enrich-tm-mw` ejecuta SIEMPRE B con flag `--full=true` (o cuando faltan dob/dorsal en >30% del roster, etc.)

**Otros hallazgos validados**:
- Slugs TM en URL son **alemanes** (`katar`, `spanien`, `schweiz`, `algerien`, `agypten`...) — pieza A los captura y los pasa a B
- TM usa lazy-load en algunos jugadores (`src=gif` + `data-src=URL`) y eager-load en otros (`src=URL directo`) — regex acepta ambos
- Foto puede ser `.jpg` o `.png`, mayúsculas o minúsculas — regex es case-insensitive
- Foto puede ser `portrait/small/`, `portrait/medium/` o `portrait/big/` — regex acepta los 3
...
export function parseRow(rowHtml) {
  ...
  // Foto: TM mezcla 2 patrones (lazy-load: src=gif placeholder + data-src=URL real;
  // eager-load: src=URL directa). Aceptamos ambos. Filtro por portrait/(small|medium|big)
  // descarta el placeholder gif. JPG/PNG ambos casos (TM usa mayúsculas y minúsculas).
  const photoMatch = rowHtml.match(/(?:data-src|src)="(https:\/\/img\.a\.transfermarkt\.technology\/portrait\/(?:small|medium|big)\/\d+-\d+\.(?:jpg|png)[^"]*)"/i);
  ...
  // El slug (X) lo guardamos también — TM usa slugs alemanes (katar, spanien, schweiz, algerien...)
  // que necesitaremos para construir URLs de kader en pieza B.
  ...
  const nation_slug = nationMatch[2];
  ...
  return {
    ...
    age,
    nation_name,
    nation_slug,                            // ← NUEVO: slug TM (alemán) para construir URL kader
...
export async function fetchAllPages({ verbose = false, useCache = true } = {}) {
  ...
  await mapConcurrent(pageNums, async (pageNum) => {
    ...
    for (const r of rows) {
      ...
      // Guardamos también el slug para construir URLs de kader (TM usa slugs alemanes).
      if (player.iso3 && player.verein_id) {
        ...
        const key = `${player.verein_id}|${player.nation_slug}`;
  ...
  // Resolver byNation: ganador es el verein_id+slug más frecuente.
  // Estructura: byNation.get(iso3) = { verein_id, slug }
  const byNation = new Map();
  for (const [iso3, counts] of vereinIdConflicts) {
    if (counts.size > 1) {
      console.warn(
        `WARN: ${iso3} con múltiples verein_id/slug TM: ${[...counts].map(([k, c]) => `${k}(×${c})`).join(', ')}`
      );
    }
    const [winnerKey] = [...counts].sort((a, b) => b[1] - a[1])[0];
    const [verein_id, slug] = winnerKey.split('|');
    byNation.set(iso3, { verein_id: parseInt(verein_id, 10), slug });
  }
...
export function parseKaderTable(html) {
  ...
  for (const r of rows) {
    ...
    // Foto desde data-src
    // Foto: TM mezcla 2 patrones (lazy-load + eager-load), .jpg/.png en mayúsculas y minúsculas.
    // Filtro portrait/(small|medium|big) descarta el placeholder gif.
    const photoMatch = rowHtml.match(/(?:data-src|src)="(https:\/\/img\.a\.transfermarkt\.technology\/portrait\/(?:small|medium|big)\/\d+-\d+\.(?:jpg|png)[^"]*)"/i);
