...
**DECISION §1.5 (review San 29-jun-2026 — rama `fix/ko-advance-ladder`):** el bonus de avance KO estaba infravalorado. Escalera coherente +5 arrancando en 10: `r32 10 · r16 15 · qf 20 · sf 25 · final 30` (`groups` sigue en 5; el podio 30/20/15/10 NO se toca). El campeon = sf 25 + final 30 + champion 30 = **85** (antes 75). Toggle reversible de 1 linea (borrar la clave `final` de `KO_ROUND_PTS` en ambos motores -> campeon = 55). Espejo `_shared/scoring.mjs` <-> `public/js/scoring.js` con paridad 1:1 en `tests/scoring.test.mjs` + `tests/ko-ia-bridge.test.mjs` + `tests/ko-standings-mesh.test.mjs`. Redeploy `get-league-standings` + reseed `user_points_cache` por lane Claude.ai tras el merge.
