...
**Fix**: eliminado el item del contador (opción A, 09-jun) y, en segunda pasada el mismo día, **neutralizados también los items A y B** (contrarian KO sobre `ko_predictions` y coincidencia campeón sobre `award_picks`): mismos SELECT capados → "Solo tú apuestas por X" salía falso con datos (el item B ya era visible en lanzamiento con 15/17 picks de campeón en Gallos; el A estaba latente hasta ~28-jun). Los slots caen al fallback genérico "Tu liga está lista para jugar". Solo sobrevive el item C (IA Zayu top 3), que lee `window._leagueRanking` — agregado legítimo de liga, no capado. **Reconstrucción correcta = deuda post-11-jun**: EF service_role agregando `award_picks`/`ko_predictions` a nivel liga con verja de membresía (patrón `get-league-standings` / `get-league-predictions` / `is_porra_abierta`).

**Patrón detectable**: `.from('<tabla own-rows-only>')` desde cliente + `.filter()`/`.length` pretendiendo agregar sobre TODA la liga. Tablas own-rows-only confirmadas en SELECT: `league_members`, `predictions`, `ko_predictions`, `boost_picks`, `award_picks`.
