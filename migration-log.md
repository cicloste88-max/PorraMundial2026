...
## 2026-06-07 — winRate relabel "12m"→"forma" + IA data refresh (rama `fix/winrate-label-forma`)

> Subsección propia (append, NO pisa #136/`awards-dorsal`). Opción A validada con San. Branch NO se mergea a main desde Code — gate de San.

[19:33] **IA data refresh vía `porra-ia-compute` (Claude.ai/MCP, service-role Bearer) — contexto de datos, NO ejecutado desde Code.**
  - `scrape_last5` `limit:10` → profundidad 8→10: 33 equipos@10, 12@9, 3@8 (ARG/QAT/URU por caché 11v11 o sin amistoso de junio). Amistosos de junio capturados: 42/48 con partido de junio, 47/48 desde 25-may.
  - `scrape_h2h` → 821 pares upserted, +6 nuevos (847 total), `matches_detail` preservado (72). Smoke OK: ARG-BRA 44-27-45/116, ARG-ESP 6-2-6/14.

[19:33] **UI relabel `winRate`: "% Victorias · 12m" → "% Victorias · forma"** (`public/js/tarjeta-stats.js`, fila de stats de la COMPARATIVA en Tarjeta·Datos del partido). Motivo: la métrica se calcula en `get-match-stats(computeWinRate12m)` sobre `ia_last5_results` (últimos 8–10 partidos); la ventana real cubre ~8,4 meses de media (rango 5–9), NO 12 → el label "12m" mentía. Decisión (Opción A, validada con San): relabelar, no perseguir un 12m real (11v11 no sirve suficientes partidos). Cambio SOLO del texto del label; `key`/`suffix`/`higherWins` intactos. `node --check` OK + `npm run build` OK + ERR-22 verificado (`Victorias · forma` presente en `dist/js/tarjeta-stats.js`, `Victorias · 12m` ausente).
  - **Paso 3 (opcional) NO aplicado**: el rename backend `computeWinRate12m`→`computeRecentWinRate` en `get-match-stats/index.ts` queda fuera de scope — la EF `get-match-stats` NO tiene fuente en el repo (`supabase/functions/` solo trae `get-league-standings`/`get-squad`/`porra-ia-compute`/`update-results`/`_shared`); es runtime-only (deploy vía MCP). Tocarlo exigiría pull+edit+redeploy de prod, no "rápido". El campo devuelto sigue siendo `winRate` (sin cambios); el nombre interno del backend sigue diciendo "12m" — relabel solo en el front (display honesto).
