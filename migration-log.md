...
## Sesión 10-jun-2026 — Hueco 1: reactivar suite cross-validate skipped (rama ci/unskip-cross-validate, Code remoto)

[12:05] **Suite cross-validate REACTIVADA — caso (b): el test estaba desactualizado, el motor está bien. Cero deploys, cero runtime.** (1) Hipótesis B2/T2 del revert #147 DESCARTADA: ni `cross-validate.mjs` ni el test cambiaron desde `bbdf1a3` (#102), y al des-skipear falla idéntico a ayer (`'high' !== 'low'`, CRO con calendar=Set(['AUT'])) — #147 tocó `update-results`, sin relación con esta suite. (2) Diagnóstico: semántica canónica de `opts.calendar` confirmada por TRES fuentes coherentes — docstring `cross-validate.mjs` L10-13 ("iso3 con '(definitiva)' FUTURA... si el iso3 no está en el set, NO degrade"), caller único `sync-squads.mjs` runDetect (pasa `pendingDefinitiveByDate` = pendientes, "solo estos se degradan") y comment de `parsers/calendar.mjs`; el test leía el set INVERTIDO (como "calendario de confirmadas", degradar al ausente). (3) Fix SOLO en el test: `tests/parsers/cross-validate.test.mjs` — test reescrito con semántica canónica (CRO EN el set de pendientes → 'low') + test complementario nuevo (CRO ausente → sigue 'high', reason undefined); `package.json` sin `--test-skip-pattern`. Suite **139/139, 0 skipped** (137 + reactivado + complementario). (4) Recordatorio boosts/17 jornadas del brief: NO aplica — la suite es cross-validation de squads (sync-squads), no toca calendario de boosts ni PARTIDOS. PR contra main SIN merge (lo gestiona Claude.ai con permiso de San). Nota: posible conflicto trivial EOF de este fichero con PR #148 (ambos appenden) — resolución keep-both.
