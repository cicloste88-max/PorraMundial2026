...
## 2026-06-09 — Saneamiento Supabase: incidente Disk IO + RLS/índices/seguridad + crones

Sesión técnica (Claude.ai + MCP Supabase, supervisa San). Proyecto `cmyfyswystjgzdwbqyyb` (eu-west-2, PG 17.6). **Doc completo: `docs/db/saneamiento-supabase-09jun2026.md`. Patrón institucionalizado: ERR-85.** Cambios SOLO en BD vía MCP (sin commits de código); este log + el doc + CHANGELOG son el rastro en git.

[~21:50 8-jun] **INCIDENTE Disk IO budget.** Dashboard: "Project is using its Disk IO budget and may become unresponsive if fully consumed". Síntomas: queries timeout, Auth `dial tcp [::1]:5432: i/o timeout`, pg_cron `job startup timeout` (hasta 628s en un solo job), `realtime.list_changes` 11.5s. Cascada 21:50→22:21 (`dispatch-live-slots` / `sweep-unbridged-finished` fallando). San hace **restart manual de Postgres 22:41 UTC** (tras upgrade de plan); estable a 22:52. Causa raíz = combinación (NO una query única): Realtime polling `live_scores` (75.8% del tiempo de PG) + RLS subóptima + multiple permissive policies + bursts de UPSERTs `predictions` (245 rows/save) + crones cada 3 min + Auth pool fijo 10 conexiones. Descartado como causa única: queries de EFs (eran síntoma; post-restart `get-squad?mode=awards`=294ms, `get-league-standings`=350-450ms).

[~22:00 8-jun] **Healthcheck previo H1-H7** (pre-incidente). H1 índice composite `predictions(league_id,match_id)`; H2 `VACUUM ANALYZE predictions` (281 dead→0); H3 `award_picks ADD COLUMN champion text` (24 rows); H4 `ko_predictions ADD COLUMN winner_team text` (786 rows); H5 advisory locks en `dispatch_live_slots()`/`sweep_unbridged_finished()` (anti-solapamiento); H6 cron 27 `purge_http_response` (retention 1h `net._http_response`); H7 cron 26 `cerrar-porras-mundial-2026` (one-shot 10-jun 21:59 UTC).

[~00:00] **8 migraciones M1-M8 vía `apply_migration` (MCP, prod, quedan en migration history).** M1 `live_scores_drop_useless_service_policy_and_dup_index` — DROP policy `live_scores_service_write` (inútil: service_role bypassRLS; re-evaluaba `auth.role()` por row) + DROP índice `live_scores_match_key_uidx` (duplicado del pkey). M2 `whatsapp_subscribers_drop_useless_service_policy` — DROP `ws_service_all` (mismo patrón; queda RLS on default-deny). M3 `squads_separate_admin_write_from_select` — DROP `squads_all_admin` (ALL solapaba SELECT) → 3 policies admin INSERT/UPDATE/DELETE con `(SELECT auth.uid())`; SELECT queda solo en la policy abierta. M4 `drop_unused_backup_tables` — DROP `equipos_players_bak_20260605` + `squads_backup_19may_premigration` (0 rows, sin FKs). M5 `enable_rls_on_h2h_scrape_map` — RLS on default-deny en tabla interna `_h2h_scrape_map`. M6 `tighten_grants_on_aggregator_views` — REVOKE ALL + GRANT SELECT solo a authenticated en `v_user_global_rank`/`v_boost_control`/`v_league_member_count` (eran ALL para anon/auth/service). M7 `revoke_execute_on_internal_security_definer_functions` — REVOKE EXECUTE de `dispatch_live_slots()`/`sweep_unbridged_finished()` (anon+auth), `trg_bridge_on_finished()` (todos, es trigger), `is_porra_abierta(uuid,uuid)` (solo anon; authenticated se mantiene por uso frontend). M8 `set_search_path_get_user_top_scorers` — `SET search_path='public','pg_temp'`. Verificación SELECT post-cada-migración.

[~00:30] **Advisors antes→después.** Performance: 5 WARN→**0** (resueltos `auth_rls_initplan`, `multiple_permissive_policies`, `duplicate_index`), 17 INFO→13 (los 13 `unused_index` NO tocar: stats reseteadas por el restart, requieren 24-48h tráfico real; `idx_boost_picks_league_id` SÍ se usa, verificado por EXPLAIN). Security: 4 ERROR→**3** (los 3 `security_definer_view` se MANTIENEN DEFINER por necesidad funcional de agregados globales — mitigado por M6; excepción documentada), 12 WARN→4 (pendientes: `is_porra_abierta` auth intencional, `unaccent` en `public`, bucket `player-photos` listing, leaked password protection), 4 INFO→5 (+1 por M5).

[~01:00] **Reactivación cron 27 `purge_http_response`:** `cron.alter_job(27, schedule:='*/30 * * * *', active:=true)` (era `*/10`). **Resto de crones (20,21,22,24,25,26) quedan PAUSADOS** pendiente decisión operativa pre-Mundial — ver §7 del doc + top-3 de CLAUDE.md (deadlines: 26 cierre porras 10-jun 21:59 UTC; 21/22 IA 11-jun 00:00/00:10; 24/25 live+puente + 20 cleanup 11-jun ~16:00 UTC pre 1er partido).

[~01:00] **Apuntes técnicos institucionalizados (→ ERR-85 + doc §9/§10):** (a) `job startup timeout` de pg_cron = pool saturado, NO cron lento. (b) `service_role` tiene `rolbypassrls=true` → policies `USING(auth.role()='service_role')` son inútiles; RLS-on sin policies = default-deny válido para tablas internas. (c) `net._http_response`/`net.http_request_queue` son UNLOGGED (sin WAL, se vacían en restart); leer SIEMPRE con `WHERE created > now()-interval` (no hay índice en `id`, owner `supabase_admin`). (d) Monitoring queries A-E en doc §9.
