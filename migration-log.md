...
## 2026-05-20 — PR #81 schema canónico squads + Sprint TM World Cup enrich

[12:30] PR #81 abierta sobre `b5f19fb` (`fix/squads-schema-canonico`). 3 commits encima de main: `3e4110b` mergeJugadores + tests (Frente 1), `f65ec20` tm-scraper schema canónico + storage-upload (Frente 2+3), `6c49821` frontend lector canónico (Frente 4). Smoke dry-run `--mode=enrich-tm --iso3=ESP --verbose` → 53 jugadores, 24 enriched, fuente `ff+tm`. Tests 34/34. Build 188.94 kB OK. PR cuerpo pegado en chat porque `gh` CLI no instalada localmente.

[13:00] PR #81 mergeada a main como squash `0be357c`. Cierra regresión 19-may donde `--mode=detect` pisaba enrich TM de FRA/SWE/BIH. `upsertSquad` hace `mergeJugadores` automático por nombre normalizado, preservando `ENRICH_FIELDS` (`tm_player_id`, `foto_url`, `edad`, `valor_eur`, `dorsal`, `dob`, `posicion_tm`). `storage-upload.mjs` sube fotos TM CDN → bucket `player-photos/{iso3}/{tm_player_id}.jpg` idempotente con Referer anti-hotlink. Frontend (`scoring.js` awards + `ui-globo-equipos.js` roster modal) actualizado al schema canónico.

[13:30] BRIEF v3 TM World Cup recibido (`BRIEF_TM_MARKET_VALUES_SCRAPER.md`, 7 piezas). Branch nueva `feat/tm-worldcup-market-values` desde main `0be357c`, orden estricto del brief, **sin squash al merge** para rollback granular.

[13:35] Pieza 0 commit `f6182cc`: `scripts/lib/tm-parse-utils.mjs` (helpers compartidos `decodeClean`/`stripTmImageQuery`/`extractProfileLink`/`extractClubLink`/`parseValorEs`/`positionToBucket`/`mapConcurrent`/`sleep`) + 10 tests. Defensa contra el bug `40,00 mill.` → `4_000_000_000` del parser viejo cubierta en `parseValorEs`. **Inadvertido**: `git add -A` cogió `BRIEF_TM_MARKET_VALUES_SCRAPER.md` (untracked) en este commit. Documentado, no force-push para evitar reescribir historia ya pusheada.

[13:45] Pieza A commit `168f9a1`: `scripts/lib/tm-worldcup-market-values.mjs` scraper FIWC 40 páginas × 25 = ~1000 jugadores, ROW_RE con lookahead positivo, `fetchAllPages` con concurrencia 4 + cache 6h en `cache/tm-mw/`, derivación `byTmId` + `byNation` (verein_id más frecuente por iso3, warning si conflict). `tm-nation-map.json` con 48 selecciones (nombres TM con acentos → iso3, lookup directo). Fixture Nuno Mendes (page 40, Portugal, PSG, 75M €) extrae 12 campos correctamente. 3 tests.

[13:50] Pieza B commit `6bb6667`: refactor completo de `parseKaderTable` en `tm-scraper.mjs` apoyado en helpers de Pieza 0. Resuelve 6 bugs del parser viejo (lookahead, data-src, DOB español, valor europeo, posición multilínea, URL `.es`). Output ampliado al schema canónico (`{tm_player_id, nombre, dorsal, foto_url_tm, posicion_tm, posicion, dob, edad, club, club_id, club_logo_url, valor_eur}`). `enrichRosterWithTm` legacy alineado a fill-missing con los nuevos field names para preservar compat con `--mode=enrich-tm`. Fixture Joan García (España, portero #18, FC Barcelona, 40M €). 2 tests.

[14:00] Pieza C-merge commit `c318e47`: `scripts/lib/enrich-merge.mjs` con `applyEnrich(roster, tmByIdMap, {iso3, sourceLabel})`. Una sola función que A y B comparten: tolera shape de A (`name`/`value_eur`/`age`/`photo_url_tm`) y de B (`nombre`/`valor_eur`/`edad`/`foto_url_tm`), persiste siempre canónico. Reglas: ID-first, fallback por nombre+iso3, **fill-missing** (no pisa valor existente), **persist-back** del tm_player_id cuando match es por nombre. Stats per-campo. 6 tests.

[14:05] Pieza D commit `cf2d1d9`: `supabase/functions/get-squad/index.ts` v6 → v7. `PlantillaPlayer` con `posicion` (bucket) + `posicion_tm` (específica TM) + `valor_eur` + `tm_player_id` + `club_logo_url`. `renderXIRow` nueva prefiere `posicion_tm` sobre `posicion` sobre fallback de formación → pizarra ahora muestra "Lateral derecho/Defensa central/Lateral izquierdo" en lugar de "Defensa/Defensa/Defensa". Alias `posicion_bucket` mantenido por retrocompat (audit del frontend confirma 0 lecturas activas).

[14:10] Pieza C-orquestador commit `ec7422b`: `--mode=enrich-tm-mw` en `sync-squads.mjs`. Fase 1: `fetchAllPages` + `maybeUpdateTmIds` (auto-fill `tm-ids.json` con verein_id descubiertos, dry-run sólo loguea). Fase 2 por país: 2a `applyEnrich(roster, byTmId, A)` → 2b si `--full` OR `coverageA<50%` OR `missingDobDorsal>30%` → `fetchTmKader(verein_id) + applyEnrich(roster, kaderMap, B)` (B aporta dorsal+dob que A no expone) → 2c upload fotos Storage idempotente throttle 200ms → 2d `upsertSquad` (mergeJugadores interno preserva resto) → 2e report tabular `iso3 | tm | val | foto | club | logo | dob | dorsal | fromA | fromB`. CLI flag `--full` añadido.

[14:15] Pieza E commit `a36c50f`: `.github/workflows/sync-squads.yml` con `enrich-tm-mw` añadido a choices del `workflow_dispatch` + nuevo input boolean `full`. Step `Run enrich-tm-mw` condicional. Cron actual sigue ejecutando `detect → enrich-tm` (no `tm-mw`) hasta validar 1 semana de dispatches manuales. Suite total 55/55 tests pass. Build OK.

[14:20] PUSH: 7 commits pusheados a `feat/tm-worldcup-market-values`. `gh` CLI no instalada, San abre PR vía pg_net. URL: `https://github.com/cicloste88-max/PorraMundial2026/pull/new/feat/tm-worldcup-market-values`. PR sin squash al merge (rollback granular). Heads-up documentado: brief `BRIEF_TM_MARKET_VALUES_SCRAPER.md` quedó incluido en Pieza 0 (commit `f6182cc`); cleanup deferred (force-push descartado).

[14:25] DOCS: `CLAUDE.md` Estado actual bumped a `0be357c` + Top-3 reordenado (PR `feat/tm-worldcup-market-values` pendiente / Sprint Reglamento FIFA / Operacional pre-launch). ERR list bumped a 01..66 + categoría iOS modal scroll (65-66). `CHANGELOG.md` con entradas `2026-05-20 — Sprint TM World Cup enrich` + `2026-05-20 — PR #81 schema canónico squads`. Este migration-log.
