...
[09:26] **send-porra-receipt v5 (DEPLOYED prod, version 5, verify_jwt=false) — proveedor email Resend → Brevo.** Cambio ACOTADO a `sendEmail()` de index.ts + secret + sender (build-data, render, idempotencia, auth, to_override, calendario grupos, KO classifier: SIN tocar). (1) Secret: lee `BREVO_API_KEY` (EF secret) en vez de RESEND_API_KEY; sigue leyendo `PORRA_FROM_EMAIL`. (2) `sendEmail()` ahora POST `https://api.brevo.com/v3/smtp/email`, headers `{api-key, accept, content-type}` (sin Bearer), body `{sender:{name,email}, to:[{email}], subject, htmlContent, attachment:[{name, content:<base64 puro>}]}` (`htmlContent` no `html`; `attachment` singular con `name` no `filename`); id de respuesta `j.messageId` → `sent_receipts.resend_id`. (3) Sender: nuevo `parseSender()` parsea `PORRA_FROM_EMAIL` ("Porra Mundial 2026 <adminmundialapp@gmail.com>") a `{name,email}`; `DEFAULT_FROM` ahora `...<adminmundialapp@gmail.com>` (remitente verificado en Brevo). Nombres legacy conservados a propósito (var `resendKey` = api-key del proveedor, columna `sent_receipts.resend_id`, error interno `resend_key_missing`) para no tocar SendConfig/processReceipt/handler. Smoke test lo hace San.
