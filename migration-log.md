...
[13:18] SCRAPLING-EVAL — análisis librería Scrapling (D4Vinci/Scrapling v0.4.7) vs stack actual (rama `claude/review-scrapling-methodology-8c6Qw`).
  - Investigación: features (adaptive selectors `auto_match`, fetchers unificados Fetcher/DynamicFetcher/StealthyFetcher, Spider con checkpoint, MCP server), anti-bot stack (curl_cffi TLS impersonation + patchright CDP-bypass + browserforge fingerprints + auto-solve Cloudflare Turnstile), benchmarks (2.02 ms / 5K elementos = paridad Parsel, ~784× vs BS4+lxml).
  - Stack actual auditado: actor `sofascore-webshare-proxy` (Node+Playwright+Webshare residencial, 5-10s, $0.001/run) + `sofascore-live-proxy` fallback + 3 scrapers HTTP en EF `porra-ia-compute` (`scrape_elo` Wikipedia API JSON, `scrape_h2h`/`scrape_last5` 11v11.com con 3 headers manuales por ERR-25, regex sobre HTML, 48 teams secuencial × 500 ms ≈ 45 s).
  - Veredicto: NO sustituir SofaScore live (caso ya óptimo); SÍ candidato para nuevo Apify actor Python que cubra H2H/last5 si 11v11 endurece anti-bot o si se añade Transfermarkt/FBref. Imposible correr Scrapling dentro de EFs (Python vs Deno).
  - Doc nuevo: `docs/scrapling-methodology.md` (16.3 KB) — TL;DR + comparativa scraper a scraper + 10 patrones extrapolables sin migrar (fetcher unificado, TLS impersonation plan B, parser semántico vs regex, cascada headers Chrome, session reuse cookies, checkpoint, multi-session routing, ProxyRotator, fingerprint plausibility, robots.txt) + plan priorizado.
  - CLAUDE.md: añadida fila `scrapling-methodology.md` al mapa de docs (10203 / 10240, margen 37 B).
