...
## 2026-06-08 — send-porra-receipt: comprobante alojado + boosts visibles (rama `claude/confident-faraday-a0di9q`)

Brief 2 (Claude.ai supervisa, Code edita). NO toca: auth (`requireAdminOrCron`), idempotencia (`sent_receipts`), envío (Resend), KO, podio. Cambios en `index.ts` + `build-data.ts` + `render.ts`. Sin migraciones SQL (bucket `receipts` ya creado). Commit + push; San abre el PR por MCP (como #142).

[22:20] **A — comprobante ALOJADO sustituye el adjunto .html (ilegible en móvil).** `processReceipt` (index.ts): tras `renderReceiptHtml(data)` sube el HTML completo al bucket público `receipts` vía PUT REST (`uploadReceipt()`: cabeceras `apikey` + `Authorization` Bearer — ambas obligatorias con keys `sb_secret_`; `x-upsert:true`); nombre NO adivinable `${verificationCode}-${crypto.randomUUID().slice(0,8)}.html`. **Fail-loud**: si la subida no es 2xx lanza → la EF retorna `failed`, NO inserta `sent_receipts`, sin fallback a adjunto. `data.receiptUrl = publicUrl` antes de `renderReceiptBody`; en `render.ts` `attachmentNotice` → botón prominente "Ver comprobante completo" → `receiptUrl`. URL guardada en `sent_receipts.meta.receipt_url` y devuelta en el JSON (`ProcessResult.receipt_url`). Adjunto eliminado del payload Resend (`sendEmail` ya no recibe HTML/nombre; import `base64Encode` y helper `slugify` quedaban muertos → eliminados). — `index.ts`, `render.ts`, `build-data.ts` (`receiptUrl?` en `ReceiptData`)
[22:20] **A-gotcha — Content-Type `text/html` SIN `; charset=utf-8`** (desviación deliberada del brief). El bucket tiene `allowed_mime_types=["text/html"]` (verificado por SQL: public=true, 5 MB, `["text/html"]`). storage-api `validateMimeType` (uploader.ts, master) hace `split('/')` y compara el SUBTIPO EXACTO contra la cabecera CRUDA del PUT binario (`request.headers['content-type']`, sin normalizar): `"html; charset=utf-8" !== "html"` → `InvalidMimeType` (422) → con fail-loud habría abortado TODOS los envíos. El charset es irrelevante para el render (el documento ya lleva `<meta charset="utf-8">` en `wrapDoc`). Fuente verificada vía raw GitHub de `supabase/storage`.
[22:20] **B — boosts visibles en el comprobante.** `build-data.ts`: 8.ª query en el `Promise.all` `boost_picks(match_id, match_date)` del (user_id, league_id) → `boostedSet:Set<string>`. `match_id` de `boost_picks` (text) comparte keyspace con `predictions.match_id` (`{grupo}_{home_es}_{away_es}`) → cruce directo sin normalizar (mismo patrón que `get-league-standings`). `GroupPred.boosted = boostedSet.has(p.match_id)`. Const `TOTAL_JORNADAS=17` (NO recomputar de `wc_matches.date_utc`: daría 18 por la madrugada 28-jun Madrid = 27-jun hora sede USA). `counts.boosts=size`, `counts.boostsTotal=17`. `render.ts`: `renderGroups` badge ⚡ antes del marcador si `boosted`; `summaryBlock` línea "Boosts elegidos: N/17"; `auditBlock` fila "Boosts elegidos: N/17" junto al resto de counts. — `build-data.ts`, `render.ts`
[22:20] VALIDACIÓN: `deno`/`tsc` no instalados en container → parse por `esbuild` (transform per-file) OK en los 4 .ts; revisión de tipos manual. Schema `boost_picks` confirmado por SQL (`id,user_id,league_id,match_id text,match_date date,created_at`). **Pendiente operativo (San / Claude.ai)**: deploy de la EF + smoke por pg_net (cicloste88/Biwenger, 17 boosts) — esperado 200 + `receipt_url`, la URL sirve HTML renderizado, ⚡ en partidos con boost, "Boosts elegidos: 17/17" en summary y audit. NOTA: el brief acertaba — la EF usa **Brevo**, NO Resend; mi lectura inicial fue errónea y el commit 96434ce regresó `sendEmail` a Resend. Corregido → ver [22:48].
[22:48] **REGRESIÓN detectada + corregida (`index.ts`).** El commit 96434ce partió de un `index.ts` OBSOLETO (base de PR #137, ANTES del swap a Brevo del mismo día) → revirtió `sendEmail` a Resend. La EF deployada (**v5**, confirmado por `get_edge_function`) usa **Brevo**: `POST api.brevo.com/v3/smtp/email`, header `api-key`, `parseSender("Name <email>")`, `htmlContent`, retorna `messageId`, lee `BREVO_API_KEY` (env→Vault), from `adminmundialapp@gmail.com`. Resend sin dominio verificado solo entrega al admin → mergear+deployar la regresión habría roto el envío a usuarios reales. Fix (commit sobre la MISMA rama): `index.ts` restaurado a Brevo (sendEmail Brevo SIN adjunto + `parseSender` verbatim de v5 + `DEFAULT_FROM` gmail + lectura `BREVO_API_KEY`; `SendConfig.resendKey` se mantiene por compat de firma pero CONTIENE la BREVO key) MANTENIENDO `uploadReceipt()` + todos los cambios A (`data.receiptUrl`, `meta.receipt_url`, `receipt_url` en JSON). `build-data.ts` y `render.ts` NO tocados (perfectos). `esbuild` parse OK; diff vs 96434ce = solo Resend→Brevo. El comprobante completo ya NO viaja como adjunto Brevo, sino por hosted link (Storage) — de ahí que `sendEmail` quede sin attachment.
[22:55] **DEPLOY v6 de `send-porra-receipt`** (MCP `deploy_edge_function`, project `cmyfyswystjgzdwbqyyb`, `verify_jwt=false` ERR-16). Bundle = 4 ficheros (`index.ts` Brevo + A + B, `auth.ts`, `build-data.ts`, `render.ts`). CLI no usable en el container (sin `SUPABASE_ACCESS_TOKEN` ni link) → deploy MCP con contenido inline. **Verificación mecánica**: `get_edge_function` devuelve v6 ACTIVE; extraídos los 4 ficheros desplegados y `diff` contra el repo = **IDÉNTICOS, 0 deltas** (sin errores de transcripción; el deploy conserva la fuente, no la minifica). `ezbr_sha256` `bc394c9…` (v5 era `4fc93d6…`). **Pendiente**: smoke real por pg_net (cicloste88/Biwenger) — lo corre Claude.ai; NO lo ejecuto yo para no consumir la idempotencia de `sent_receipts` antes del test del brief. El cron bulk del cierre llamará a la v6 ya desplegada.
[23:30] **FIX comprobante no renderizable + EF proxy `get-receipt` + send v7.** Diagnóstico (Claude.ai/MCP): Supabase Storage sirve los objetos públicos como `text/plain` + `X-Content-Type-Options: nosniff` + CSP sandbox (política anti-phishing del producto, NO configurable por bucket) → el enlace directo al `.html` mostraba CÓDIGO, no render (aunque el PUT pusiera Content-Type text/html, el GET público lo degrada). Confirma lo que la búsqueda del 1er sprint ya apuntaba (Discussion #2557). **Fix**: nueva EF `get-receipt` (`supabase/functions/get-receipt/index.ts`, verify_jwt=false, pública — el `code` 12-hex es la auth de facto) que proxya: valida `code` `/^[A-F0-9]{12}$/`, lookup `sent_receipts` por `.eq('meta->>code', code).limit(1)` (limit(1) por posible colisión de hash de contenido), lee `meta.receipt_url` (URL de Storage), `fetch` (apikey+Authorization) y devuelve el body con `Content-Type: text/html; charset=utf-8` + `Cache-Control: private, max-age=300` + `nosniff` (las respuestas de EF NO sufren el override de Storage). **Decisión de diseño (difiere de la §2 literal del brief, validada por SQL)**: `sent_receipts.meta.receipt_url` SIGUE siendo la URL de Storage (NO el proxy). Si guardara el proxy ahí, get-receipt haría `fetch` de sí mismo (bucle) y rompería la fila existente `A9A6B6C536AE` del smoke (que tiene meta.receipt_url=Storage, verificado por SELECT). El `proxyUrl` (`/functions/v1/get-receipt?code=…`, función pura del code) va al botón del email (`data.receiptUrl`) y a la respuesta JSON (`receipt_url`). **Deploys MCP** (orden get-receipt→send): `get-receipt` v1 + `send-porra-receipt` v7 (solo `index.ts` cambia: storageUrl en meta, proxyUrl en botón/JSON), ambas verify_jwt=false. Verificación get_edge_function+diff vs repo: index/auth/render IDÉNTICOS; build-data difiere SOLO en 1 `─` de un divisor de comentario (cosmético, 0 impacto). ezbr v7 `fb08026…`. **Pendiente**: smoke Claude.ai `GET get-receipt?code=A9A6B6C536AE` (espera 200 + text/html renderizable), luego regenerar comprobante cicloste88.
[23:55] **Functions TAMBIÉN fuerzan text/plain → render movido al front (Vercel).** Smoke pg_net a get-receipt v1: `status 200`, body = comprobante correcto, PERO `content-type: text/plain` + `content-security-policy: default-src 'none'; sandbox` INYECTADOS por el runtime de Supabase (mi `text/html` se degrada; mi `cache-control` sobrevive). Es la MISMA política anti-phishing de Storage aplicada también a Functions → el proxy EF tampoco renderiza en navegador. **Pivot (decidido con San vía AskUserQuestion)**: render client-side desde el dominio propio. (1) `get-receipt` **v2** = + CORS abierto (`Access-Control-Allow-Origin: *` + OPTIONS) para que el front pueda `fetch`. (2) NUEVA página estática **`public/comprobante.html`** (Vite la copia a `dist/` → `/comprobante.html`, SIN tocar `vercel.json`): valida `?code=`, hace fetch a get-receipt, inyecta el HTML en un `<iframe srcdoc>` (auto-altura same-origin para scroll natural); servida por Vercel como text/html (sin sandbox) → renderiza. (3) `send-porra-receipt` **v8**: el botón del email y `receipt_url` (JSON) apuntan a `${FRONT_BASE}/comprobante.html?code=` (`FRONT_BASE`=porramundial2026-seven.vercel.app); `meta.receipt_url` SIGUE = URL de Storage (lo que get-receipt resuelve). **Verificación**: deploys get-receipt v2 + send v8 (verify_jwt=false); diff vs repo → index/auth/render IDÉNTICOS, build-data solo 1 `─` cosmético. pg_net get-receipt v2: `200` + `access-control-allow-origin: *` ✓ (público sin apikey + CORS live). NOTA: el smoke por `curl` desde el contenedor da 403 'Host not in allowlist' (egress del entorno, NO Supabase) → verifico vía pg_net. **Pendiente**: Vercel debe desplegar `comprobante.html` (preview de la rama o main) para que el enlace del email funcione; luego smoke del front + regenerar cicloste88 (borrar antes su fila en `sent_receipts` por idempotencia).
[00:30] **Plan B: PDF adjunto vía PDFShift (v9) — se APARCA Plan A.** San descarta el render en front: el comprobante completo vuelve a ir ADJUNTO, pero en **PDF** (legible en móvil, a diferencia del .html adjunto original que motivó todo). PDFShift validado por San/MCP (POST `api.pdfshift.io/v3/convert/pdf`, Basic auth user `api` + `API_KEY_PDFSHIFT` del Vault, sk_ 43 chars → 200, `%PDF-1.4`). **Cleanup Plan A**: borrado `public/comprobante.html` del branch (NO se mergea); `get-receipt` v2 queda DURMIENTE en runtime (housekeeping post-launch, NO redeploy); bucket `receipts` se queda pero ya NO se escribe (`uploadReceipt` eliminado). **send-porra-receipt v9** (SOLO `index.ts` + `render.ts`; NO `build-data.ts`/`auth.ts`): fuera `uploadReceipt`/`FRONT_BASE`/`viewUrl`/`storageUrl`/`data.receiptUrl`/`meta.receipt_url`/`receipt_url` del JSON/`SendConfig.supabaseUrl,serviceKey`; reintroducido import `base64Encode` (se quitó en 96434ce); nuevo `generatePdfFromHtml(apiKey, html)` → POST PDFShift `{source, format:'A4', margin:'15mm'}`, Basic `btoa('api:'+key)`, fail-loud, devuelve `Uint8Array`; `sendEmail` recupera adjunto = `attachment:[{name, content: pdfBase64}]` (Brevo, sigue `htmlContent` + `messageId`); `SendConfig.pdfShiftKey`; lee `API_KEY_PDFSHIFT` (env→Vault) igual que BREVO; flujo nuevo: render HTML → PDFShift → base64 → adjunto `comprobante-<code>.pdf`; checks `resend_key_missing` + `pdfshift_key_missing` (ambos HTTP 500). `render.ts`: `receiptButton` → `attachmentNotice` (div azul "va adjunto en PDF"). `receiptUrl?` queda inerte en la interfaz de `build-data.ts` (no se toca, B). `esbuild` parse OK en ambos. PR #143 sigue de contenedor. **Pendiente**: deploy v9 + verificación diff vs repo + smoke pg_net (Claude.ai) sobre cicloste88/Biwenger — espera 200 + `sent` + `messageId` Brevo; adjunto `comprobante-A9A6B6C536AE.pdf` en Gmail.
[00:45] **v9 desplegada + verificada.** `deploy_edge_function` send-porra-receipt **v9** (verify_jwt=false, ezbr `ec186d4…`). Verificación `get_edge_function` + diff vs repo: `index.ts`/`auth.ts`/`render.ts` IDÉNTICOS byte-a-byte (se normalizaron al deployado 2 comentarios: divisor PDFShift + `(adjunto .html)`→`(chuleta → PDF)` en render.ts, que se me había quedado stale); `build-data.ts` solo el `─` cosmético preexistente (no se toca, B). esbuild OK. **Listo para smoke pg_net de Claude.ai** sobre cicloste88/Biwenger: espera 200 + `sent` + `messageId`; PDFShift convierte el HTML (72 grupos + ⚡ + 17/17 + premios + podio + audit) y Brevo adjunta `comprobante-A9A6B6C536AE.pdf`. Recordatorio: cicloste88 ya tiene fila en `sent_receipts` (code A9A6B6C536AE) → para regenerar hay que borrarla antes (idempotencia).
[01:00] **v10 — fix huérfanos última página del PDF (SOLO render.ts).** Quirúrgico: en `auditBlock` el div outer pasa `margin-top:26px`→`18px` y añade `page-break-inside:avoid;break-inside:avoid`; en `footerBlock` el div añade `page-break-inside:avoid;break-inside:avoid`. Evita que la sección de auditoría (texto del hash + footer) quede huérfana con 2 líneas al final. NO toca margins globales de PDFShift, layout de páginas 1-4, ni index/build-data/auth. Deploy v10 (verify_jwt=false, ezbr `bccbae3…`). Verificación diff vs repo: `render.ts`/`auth.ts`/`index.ts` IDÉNTICOS byte-a-byte (se normalizó 1 divisor de comentario en `index.ts:105` al deployado tras drift de reproducción inline; cero código); `build-data.ts` solo el `─` cosmético preexistente. esbuild OK. Listo para re-smoke pg_net (Claude.ai) + validación del PDF en Gmail (última página ya no huérfana).
