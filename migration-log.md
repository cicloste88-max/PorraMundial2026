...
[22:40] FIX prod auth bootstrap congelado tras refresh (rama NUEVA `fix/auth-bootstrap-frozen-refresh` desde `main f626714` — el PR#124 ya está mergeado a main). Bug reportado por San + reproducido por Claude.ai vía Chrome MCP: tras F5 / recarga (iPhone + Android), el header se ve (ADMIN, usuario, "Cerrar sesión") pero el resto vacío (azul liso, sin grupos, sin nav). Workaround actual del usuario: logout+login. Causa (3 fallos compuestos en `runAuthInit > onAuthStateChange` branch `INITIAL_SESSION` de `public/js/auth.js`): (1) `await leagueLoadMyLeagues()` SIN retry — fetch transitorio falla, `_myLeagues` queda `[]`, `.find()` no encuentra la liga guardada, NO entra en `leagueSelectById`, ningún `showPage` se llama; (2) sin timeout en los awaits — un fetch colgado (Supabase sin `signal` nativo) deja el handler pending para siempre; (3) early-return `if (found) {...; return;}` sale sin red de seguridad si algo falla entre `find()` y `leagueSelect`. Sintoma: shell montado pero todos los hijos a `height:0`. Cura manual del reporte: `window.leagueLoadMyLeagues()` consola resuelve en 862ms (red funciona, solo el primer intento se ahogó). Fix: (A) helper `_withTimeout(promise, ms, label)` Promise.race aplicado a profile fetch (8s) + leagueLoadMyLeagues (8s/intento) + leagueSelectById (8s) + loadUserData (10s). (B) Retry con backoff 0/400/800/1600ms (4 intentos) sobre leagueLoadMyLeagues. (C) Flag `_navigated` + `try/finally` garantiza `showPage` en TODOS los caminos. (D) Preservar `savedLeagueId` si tras 4 intentos `_myLeagues` sigue vacío (posible transient — no borrar datos legítimos del usuario); solo limpia si _myLeagues no vacío pero la guardada no está (stale id legítimo). (E) Loader visible `#_auth-bootstrap-loader` (fixed center, CSS inline) si hay `sessionStorage.porra_token` o `_pendingPageRestore` — removido al primer `_markNavigated()`. (F) Watchdog 12s fuerza `showPage('welcome')` como red extrema. Guards intactos: TOKEN_REFRESHED/USER_UPDATED early-return (línea 431) + `currentUser.id===session.user.id` (línea 445) — fix convive sin pisarlos. 1 fichero tocado. Sin tocar BBDD, EF, leagues.js ni `runAuthInit` fuera del scope del handler. Docs: ERR-78 + CHANGELOG. Pendiente: QA en preview Vercel por San (lección PR#124: test standalone NO captura timing real de fetch).
