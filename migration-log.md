...
[01:04] **Fix highlights: contador "porras cerradas/pendientes" eliminado (ERR-85).** "DESTACADOS DE TU LIGA" mostraba "0 cerradas / 1 pendientes" en toda liga: la policy `lm_select` de `league_members` (`auth.uid()=user_id`) capa el SELECT de cliente a la fila propia → contador irrealizable client-side (Gallos real: 15/2 de 17). Opción A aplicada: item retirado de `loadLeagueHighlights` (`data.js`), slot cae al genérico "Tu liga está lista para jugar". Verificadas vía MCP las policies SELECT own-rows-only de `league_members`/`ko_predictions`/`award_picks` → items A/B de la misma función comparten el cap (señalados en ERR-85, sin tocar). CHANGELOG + catálogo ERR-85 actualizados. Sin cambios EF/RLS/CSS.

[01:14] **Fix highlights (cont.): items A y B de `loadLeagueHighlights` neutralizados (ERR-85, opción 1 aprobada San).** Mismo tratamiento que el contador: bloques de `ko_predictions` (contrarian KO, "Solo tú apuestas por X en {ronda}") y `award_picks` (coincidencia campeón, "Solo tú apuestas por X campeón") eliminados de `data.js` — ambas tablas SELECT own-rows-only (`auth.uid()=user_id`), el cliente solo ve su fila y los "solo tú" salen falsos. Sustituidos por comentario-guard → ERR-85. Item C (IA Zayu, `window._leagueRanking`) INTACTO. `var db` muerto retirado (la función ya no consulta BD). Slots caen al genérico "Tu liga está lista para jugar". ERR-85 ampliado (A/B neutralizados + reconstrucción EF service_role = deuda post-11-jun) + CHANGELOG 09-jun actualizado. Sin tocar EF/RLS/CSS. Gate: node --check + build + grep dist. QA localhost: San (1 ciclo) → luego PR+merge.
